...
const VideoTile: React.FC<VideoTileProps> = ({ userId, isFocused, onFocus }) => {
  ...
  // CRITICAL SAFETY CHECK: If state is missing, don't render
  if (!voiceState) return null;

  const stream = voiceState.stream;
  const hasVideo = !!stream && stream.getVideoTracks().length > 0;
  const isSpeaking = (voiceState.volume || 0) > 0.01 && !voiceState.isMuted;
    ...
    >
      {hasVideo ? (
        ...
        <div className="tile-avatar-container">
            <div className="tile-avatar" style={{ backgroundColor: generateAvatarColor(userId) }}>
